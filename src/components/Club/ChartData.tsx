import { Type } from "@/types/chart";


const ChartData: Type[] = [
  {
    id: 1,
    title: "COMITE DE DIRECTION",
    persons: [
      {
        name: "Magalie GIBOULET",
        image: "/images/chart/magalie.jpg",
        designation: "Présidente",
        mail: "presidence@fcfranchevillois.fr",
        phone: "04 72 16 01 38"
      },
      {
        name: "-",
        image: "/images/logo/logo-spaced.png",
        designation: "Secrétaire Général",
        mail: "secretariat@fcfranchevillois.fr",
        phone: "04 72 16 01 38"
      },
      {
        name: "Moufid HFSAOUI",
        image: "/images/chart/moufid.jpg",
        designation: "Trésorier",
        mail: "tresorerie@fcfranchevillois.fr",
        phone: "04 72 16 01 38"
      },
      {
        name: "Gilles OLLAGNIER",
        image: "/images/chart/gilou.jpg",
        designation: "Correspondant",
        mail: "secretariat@fcfranchevillois.fr",
        phone: "06 35 22 33 92"
      },
    ]
  },
  {
    id: 2,
    title: "TECHNIQUES / SPORTIFS",
    persons: [
      {
        name: "Andy BOUHADA",
        image: "/images/chart/andy.jpg",
        designation: "Responsable Technique Foot A11",
        mail: "technique@fcfranchevillois.fr",
        phone: "06 26 84 16 41"
      },
      {
        name: "Corey BUGNET",
        image: "/images/chart/corey.jpg",
        designation: "Responsable Technique École de Foot",
        mail: "technique@fcfranchevillois.fr",
        phone: "07 85 31 12 75"
      },
    ]
  },
  {
    id: 3,
    title: "ENCADREMENT",
    persons: [
      {
        name: "Alain DEYRAIL",
        image: "/images/logo/logo-spaced.png",
        designation: "Coach Senior 1",
        mail: "",
        phone: "06 78 85 15 99"
      },
      {
        name: "Charles DARCY",
        image: "/images/chart/charles.jpg",
        designation: "Coach Senior 2",
        mail: "",
        phone: "06 26 87 84 73"
      },
      {
        name: "Gilles OLLAGNIER",
        image: "/images/chart/gilou.jpg",
        designation: "Educateur U20",
        mail: "",
        phone: "06 35 22 33 92"
      },
      {
        name: "Corey  BUGNET",
        image: "/images/chart/corey.jpg",
        designation: "Educateur U17",
        mail: "",
        phone: "07 85 31 12 75"
      },
      {
        name: "Sandrine HOSNI",
        image: "/images/chart/sandrine.jpg",
        designation: "Educatrice U15",
        mail: "",
        phone: "06 20 57 36 69"
      },
      {
        name: "Clément DURAND",
        image: "/images/chart/clement.jpg",
        designation: "Educateur U13",
        mail: "",
        phone: "06 75 16 31 40"
      },
      {
        name: "David TOYOU",
        image: "/images/chart/david.jpg",
        designation: "Educateur U12",
        mail: "",
        phone: "06 20 08 29 98"
      },
      {
        name: "Issa SISSOKO",
        image: "/images/chart/issa.jpg",
        designation: "Éducateur École de foot",
        mail: "",
        phone: "07 84 83 99 39"
      },
    ]
  },
  {
    id: 4,
    title: "BENEVOLAT",
    persons: [
      {
        name: "Raphaël MESTRE",
        image: "/images/chart/raphael.jpg",
        designation: "Apprenti communication",
        mail: "",
        phone: ""
      },
      {
        name: "Amine ZAKI",
        image: "/images/chart/amine2.jpg",
        designation: "Service civique",
        mail: "",
        phone: ""
      },
      {
        name: "Cyril MOUSSOUNI",
        image: "/images/chart/cyril.jpg",
        designation: "Dirigeant",
        mail: "",
        phone: ""
      },
      {
        name: "Philippe MATHYS",
        image: "/images/chart/philippe.jpg",
        designation: "Dirigeant",
        mail: "",
        phone: ""
      },
      {
        name: "Damien BECHET",
        image: "/images/chart/damien.jpg",
        designation: "Dirigeant",
        mail: "",
        phone: ""
      },
    ]
  }
];
export default ChartData;
